...
export default function IconTextButton({title, color, onPress, icon, textColor, large, link}: {title: string, color: string, onPress?: () => void, icon: IconSymbolName, textColor: string, large?: boolean, link?: string}) {

    return (
        link?
            <Link asChild style={[styles.button, { backgroundColor: color, borderColor: textColor, height: large? 50 : "auto" } ]} href={link}>

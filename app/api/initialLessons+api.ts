import flatLessons from '../../db/flatLessons.json'
...
export async function GET(request: Request) {
  ...
  const courseIds = courseParam
    ...
    : [];
      ...
      // Filter by courseId
  const userCards = flatLessons.contents.filter(lessons =>
    courseIds.includes(lessons.courseId)
  );

  // Filter to show only first deck for each courseId
  const data = userCards.filter( flashcard =>
    flashcard.lessonId < 5 // Assuming lessonId 1 is the first deck
  )
